...
impl Tensor {
    ...
    pub fn zeros(shape: &[usize]) -> Self {
        Tensor::new(ndarray::Array::zeros(IxDyn(shape))).requires_grad(false)
    }

    pub fn ones(shape: &[usize]) -> Self {
        Tensor::new(ndarray::Array::ones(IxDyn(shape))).requires_grad(false)
    }

    pub fn randn(shape: &[usize]) -> Self {
        Tensor::new(Array::random(IxDyn(shape), StandardNormal)).requires_grad(false)
    }
